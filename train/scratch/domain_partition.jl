...
function set_initial_geometry(x)
	if (x < 0.2) 1 else 0. end
end
@register set_initial_geometry(x)

function set_boundaries(t_0)
    bcs = [f(t_0,x,v) ~ set_initial_geometry(x) * 1/(v_th * sqrt(2π)) * exp(-v^2/(2*v_th^2)), E(t_0,x) ~ set_initial_geometry(x) * e*n_0/ε_0 * (Iv(f(0,x,v)) - 1)]
end
...
for i in initθ_
    push!(initθ, map(c -> Float64.(c), initθ_[1]))
end
...
for i in 1:count_decomp
    ...
    res_ = GalacticOptim.solve(prob, BFGS(); cb=cb, maxiters=1)
...
losses = map(1:count_decomp) do i
    minimizers_ = [reses[i].minimizer[s] for s in sep]
    loss(cord,θ) = chain2(cord,θ) .- phis[i](cord,minimizers_)
end
